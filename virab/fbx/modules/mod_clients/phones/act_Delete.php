<?php
	...
	if(!$auth_in->aclCheck($resourceId, DELETE)){
		...
		Location(sprintf($_XFA['phones'], $id_client, $ACL_ERROR), 0);
		die;
	}
	...
	function delElement($id) {
		...
		if ($id) {
			$sql = sql_placeholder("DELETE FROM " . CFG_DBTBL_MOD_PHONES_STORAGE . " WHERE id = ?", $id );
			$db->query($sql);
			$sql = sql_placeholder("DELETE FROM " . CFG_DBTBL_MOD_CLIENT_PHONES . " WHERE id_phone_storage = ?", $id );
			$db->query($sql);
		} else {
	...
	Location(sprintf($_XFA['phones'], $id_client, $FORM_ERROR), 0);
