...
/*global define, window, document, URL, webkitURL, FileReader */
	...
	**/
var blink_maxcount = 3;
var blink_count = 0;
function blinkborder(i)
	{
	$(i).animate({ 
		backgroundColor: '#DFFFB2'
		}, 200, function() {
			$(i).animate({ 
			backgroundColor: '#FFF'
			}, 200, function() {
				blink_count++;
				if(blink_count < blink_maxcount)
					blinkborder(i);
		});
	});			
	} 
function editAltPhotoNew(id_photo, i) {
	$.ajax({
		type: "POST",
		url: BackEndURLS.main_photo_astore,
		data: "photo_alt_store=1&id_photo=" + id_photo + "&new_text=" + i.value,
		success: function(msg){
			//alert('Новое значение успешно сохранено');
			blink_count = 0;
			blinkborder(i);
		}
	});
...
(function (factory) {
    ...
    $.widget('blueimpUI.fileupload', parentWidget, {
        ...
        _transition: function (node) {
        	$( ".file_upload_wrapper .files" ).sortable( "destroy" )
        	$( ".file_upload_wrapper .files" ).sortable({
			...
			$('.sort_child').each(function(){
				...
				$(this).find( ".name_change" ).change(function(){
					editAltPhotoNew(id, this);
					});
				});			
