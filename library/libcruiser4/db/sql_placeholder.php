...
//   ?* - заменяется на список пар ключ=значение: k1='v1' AND k2='v2' AND ...
//
    ...
    do {
      ...
      if ($type === '@') {
        ...
        if (count($lerror)) {
      	...
      	// Это набор пар ключ => значение, разделенные AND
        $lerror = array();
        foreach ($a as $k=>$v) {
          if (!is_string($k)) {
            $lerror[$k] = "NOT_A_STRING_KEY_{$k}_FOR_PLACEHOLDER_$key";
...
/**
 * Placeholder функция для SQL запросов
 *
 * Заменяет все placeholder-ы в $tmpl на их SQL-экранированные значения
 * из $args. При ошибке сохраняет диагностическое сообщение в $errormsg.
 *
