<?php
	...
	function mailto($email, $my_visuality = false, $visual = '', $params = array()) {
		...
		if (!$decode_func_sent) {
			...
			?>
				...
				<script type="text/javascript">function <?=$decode_func_name?>(d) {var d = eval(decodeBase64(d));var a = [];a.length = d[0].length;for (var i=0; i<d[0].length; i++) {a[d[0][i]] = d[1].charAt(i);}return a.join('');}</script>
			<?php
	...
	*/
	...
	/**
	 * Функция проверки формата email адреса
	 *
	 * @param string $email
	 * @return boolean
	 */
