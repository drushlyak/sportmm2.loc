...
/**
 * @package MilKit_Acl
 * @author Zend Framework contributors
 * @author Marat Komarov
 * @author Serge (.ter) Red`kin [rou.terra@gmail.com]
 */
 ...
 */
	 ...
	 */
	require_once (MAIN_LIB_PATH . "/Zend/Acl/Resource/Interface.php");
	
	
	/**
	 * MilKit_Acl_Role_Registry
	 */
	require_once (MAIN_LIB_PATH . "/MilKit/Acl/Role/Registry.php");
	
	
	/**
	 * Zend_Acl_Assert_Interface
	 */
	require_once (MAIN_LIB_PATH . "/Zend/Acl/Assert/Interface.php");
	 ...
	 * MilKit_EventManager
	 */
	require_once (MAIN_LIB_PATH . "/MilKit/EventManager.php");
	
	/**
	 * Класс управления ACL
	 * 
	 * @package MilKit_Acl
	 */
	class MilKit_Acl {
	    ...
	    /**
	     * @var MilKit_Acl_Store_Interface
	     */
	    protected $_store;
	     ...
	     * Flags
	     */
	    const ANY_RESOURCE  = 0x01;
	    const ANY_ROLE 		= 0x02;
		const ANY_PRIVILEGE = 0x04;
		const ANY_TYPE		= 0x08;
	
		/**
		 * Конструктор класса
		 * 
		 * @var MilKit_Acl_Store_Interface $store
		 */
	    public function __construct ($store=null) {
	    	$this->_eventManager = new MilKit_EventManager();
	    	$this->_eventManager->addEvents(array(
	    	
	    		/**
	    		 * Rule was added/changed
	    		 * @param mixed $resourceId
	    		 * @param mixed $roleId
	    		 * @param mixed $privilegeId
	    		 * @param mixed $type
	    		 */
	    		'setrule',
	    	...
	    	$evm = $this->_getRoleRegistry()->getEventManager();
	    	$evm->on('add', array($this, 'onAddRole'));
	    	$evm->on('remove', array($this, 'onRemoveRole'));
			$evm->on('removeall', array($this, 'onRemoveRoleAll'));
			if ($store !== null) {
				$this->setStore($store);
	    	}
	    }
	    ...
	    public function setStore ($store) {
	    	if ($this->_store) {
	    		$this->_store->unbindAcl();
	    	}
	    	$this->_store = $store;
	    	$store->bindAcl($this);
	    	$this->_eventManager->suspendEvents();
	    	$store->load();
	    	$this->_eventManager->resumeEvents();
	    }
	            ...
	            foreach ($visitor['byRoleId'] as $roleIdCurrent => $rules) {
	                if ((int) $roleId === (int) $roleIdCurrent) {
	                    ...
	                    $this->_eventManager->fire(
	                    	'removerule',
	                    	self::ANY_PRIVILEGE | self::ANY_TYPE,
	                    	$resourceIdCurrent, $roleIdCurrent
	                    );
	                    //
	                }
	            }
	    ...
	    public function add(Zend_Acl_Resource_Interface $resource, $parent = null) {
	        ...
	        if ($this->has($resourceId)) {
	            require_once (MAIN_LIB_PATH . "/Zend/Acl/Exception.php");
			...
			$this->_eventManager->fire('addresource', $resource, $resourceParent ? $resourceParent->getResourceId() : null);
	     ...
	     * @return Zend_Acl Provides a fluent interface
	     */
	    public function remove($resource, $removeChild = true) {
	    	
	    	// [RESID0TERHACK] Хак, позволяющий обойти необходимость создания значения в дереве ресурсов
	    	// для res_id = 0 ACL проверка пропускается
			if ($resource == '0') {
				return $this;
			}
	        ...
	        foreach ($resourcesRemoved as $resourceIdRemoved) {
	        	foreach ($this->_rules['byResourceId'] as $resourceIdCurrent => $rules) {
	        		if ($resourceIdRemoved === $resourceIdCurrent) {
	                    ...
	                    $this->_eventManager->fire(
	                    	'removerule',
	                    	self::ANY_ROLE | self::ANY_PRIVILEGE | self::ANY_TYPE,
	                    	$resourceIdCurrent
	                    );
	                }
	            }
	        ...
	        $this->_eventManager->fire('removeresource', $resource, $removeChild);
	        return $this;
	    }
	    ...
	    public function removeAll() {
	        ...
	        $this->_eventManager->fire('removeresourceall');
	        return $this;
	    }
	    ...
	    public function setRule($operation, $type, $roles = null, $resources = null, $privileges = null,
	        ...
	        // [RESID0TERHACK] Хак, позволяющий обойти необходимость создания значения в дереве ресурсов
	    	// для res_id = 0 ACL проверка пропускается
			if ($resources === 0 || $resources === '0' || $resources === array(0) || $resources === array('0')) {
				return $this;
			}
	            ...
	            case self::OP_ADD:
	            	foreach ($resources as $resource) {
	                    foreach ($roles as $role) {
	                        ...
	                        // Patch
							$evResource = $resource ? $resource->getResourceId() : null;
							$evRole = $role ? $role->getRoleId() : null;
							//
	                        if (0 === count($privileges)) {
	                            ...
	                            // Patch
	                            $this->_eventManager->fire('setrule', $evResource, $evRole, null, $type);
	                            //
	                        } else {
	                            foreach ($privileges as $privilege) {
	                                ...
	                                // Patch
	                                $this->_eventManager->fire('setrule', $evResource, $evRole, $privilege, $type);
	                                //
	                            }
	                        }
	                    }
	                        ...
	                        if (0 === count($privileges)) {
	                            if (null === $resource && null === $role) {
	                                if ($type === $rules['allPrivileges']['type']) {
	                                    ...
	                                    // Patch
	                                    $this->_eventManager->fire(
	                                    	'removerule', 0, null, null, null, $type
	                                    );
	                                    $this->_eventManager->fire(
	                                    	'setrule', $evResource, $evRole, null, self::TYPE_DENY
	                                    );
	                                    //
	                            ...
	                            if ($type === $rules['allPrivileges']['type']) {
	                                ...
	                                $this->_eventManager->fire('removerule', 0, $evResource, $evRole, null, $type);
	                            }
	                        } else {
	                            foreach ($privileges as $privilege) {
	                                if (isset($rules['byPrivilegeId'][$privilege]) &&
	                                    ...
	                                    // Patch
	                                    $this->_eventManager->fire(
	                                    	'removerule', 0, $evResource, $evRole, $privilege, $type
	                                    );
	                                    //
	                                }
	                            }
	     ...
	     *
	     * @return MilKit_Acl_Role_Registry
	     */
	    protected function _getRoleRegistry() {
	        if (null === $this->_roleRegistry) {
	            $this->_roleRegistry = new MilKit_Acl_Role_Registry();
	        }
	    ...
	    public function onAddRole ($role, $parentsId) {
	    	$args = func_get_args();
	    	array_unshift($args, 'addrole');
	    	call_user_func_array(array($this->_eventManager, 'fire'), $args);
	    }
	
	    public function onRemoveRole () {
	    	$args = func_get_args();
	    	array_unshift($args, 'removerole');
	    	call_user_func_array(array($this->_eventManager, 'fire'), $args);
	    }
	
	    public function onRemoveRoleAll () {
	    	$args = func_get_args();
	    	array_unshift($args, 'removeroleall');
	    	call_user_func_array(array($this->_eventManager, 'fire'), $args);
	    }
	
	    public function hasOwnRule ($type, $resource=null, $role=null, $privilege=null) {
	        if (null !== $role) {
	            $role = $this->_getRoleRegistry()->get($role);
	        }
	        ...
	        return $this->_getRuleType($resource, $role, $privilege) === $type;
	    }
	}
