<?php
	...
	if(!$auth_in->aclCheck($resourceId, DELETE)){
		...
		Location(sprintf($_XFA['bonus'], $id_client, $ACL_ERROR), 0);
		die;
	}
	...
	function delElement($id) {
		...
		if ($id) {
			$sql = sql_placeholder("DELETE FROM " . CFG_DBTBL_MOD_CLIENT_BONUS . " WHERE id = ?", $id );
			$db->query($sql);
	...
	Location(sprintf($_XFA['bonus'], $id_client, $FORM_ERROR), 0);
