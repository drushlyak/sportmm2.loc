<?php
	...
	if(!$auth_in->aclCheck($resourceId, DELETE)){
		...
		Location(sprintf($_XFA['cp_phones'], $id_client, $id_contact, $ACL_ERROR), 0);
		die;
	}
	...
	function delElement($id) {
		...
		if ($id) {
			...
			$db->query($sql);
			$sql = _psql("DELETE FROM " . CFG_DBTBL_MOD_CONTACT_PERSON_PHONES . " WHERE id_phone_storage = ?", $id );
			$db->query($sql);
		} else {
	...
	Location(sprintf($_XFA['cp_phones'], $id_client, $id_contact, $FORM_ERROR), 0);
